...
def control_robot(pub, forward, speed=0.2):
    print("control_robot test")
    twist = Twist()
    if forward:
        twist.linear.x = speed  # Avancer
    else:
        twist.linear.x = -speed  # Reculer
    twist.angular.z = 0.0
    pub.publish(twist)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if obstacle_detected:
                ...
                # Check if both left and right sensors detect an obstacle
                if distance_left < threshold and distance_right < threshold:
                    print("Both left and right obstacles detected, reversing")
                    control_robot(pub, forward=False, speed=0.2)  # Reverse
                    rospy.sleep(2)  # Reverse for 2 seconds
                    stop_robot(pub)  # Stop after reversing
                else:
                    # Determine the direction to turn
                    if distance_left > distance_right:
                        turn(90, pub)  # Turn left by 45 degrees
                        print("Turning left by 45 degrees")
                    else:
                        turn(-90, pub)  # Turn right by 45 degrees
